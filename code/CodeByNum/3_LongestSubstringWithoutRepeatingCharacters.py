"""
    ...
    需要注意: Python中用in来查找item是否存在于temp中，这里的时间复杂度应该是O(len(temp))，所以算法的总时间复杂度应该是O(n * len(temp))
    time: O(n * len(temp)) space: O(len(temp))
...
"""
    ...
    需要注意: 用in去查找c是否存在于哈希表中，时间复杂度是O(1),所以算法的总时间复杂度是O(n)
